...
class TextFormatter {
  ...
  static List<TextSpan> formatContent(String content,
    ...
    if (kDebugMode)
      print(
          'DEBUG: Raw content (first 200 chars): ${content.substring(0, content.length > 200 ? 200 : content.length)}');
    ...
    final doubleQuotePatternDash =
        RegExp(r'"([^"]+?)"(\.?)-(' + verseRefPattern + r')(\s|\n\n|\n|$)');
    final singleQuotePatternDash =
        RegExp(r"'([^']+?)'(\.?)-(" + verseRefPattern + r')(\s|\n\n|\n|$)');
    ...
    final doubleQuotePatternSpace = RegExp(
        r'^"([^"]+?)"(\.?)\s+(' + verseRefPattern + r')(\s|\n\n|\n|$)',
        multiLine: true);
    final singleQuotePatternSpace = RegExp(
        r"^'([^']+?)'(\.?)\s+(" + verseRefPattern + r')(\s|\n\n|\n|$)',
        multiLine: true);
    ...
    content = content.replaceAllMapped(
      ...
      (match) {
        ...
        if (kDebugMode)
          print(
              'DEBUG: Replacing double quote with dash #$replacementCount: "$verseText"$period-$reference');
    ...
    content = content.replaceAllMapped(
      ...
      (match) {
        ...
        if (kDebugMode)
          print(
              'DEBUG: Replacing single quote with dash #$replacementCount: \'$verseText\'$period-$reference');
        // Always put double newline after reference to separate it from body text
        final replacement = "'$verseText$period'\n$reference\n\n";
        return replacement;
      },
    ...
    content = content.replaceAllMapped(
      ...
      (match) {
        ...
        if (kDebugMode)
          print(
              'DEBUG: Replacing double quote with space #$replacementCount: "$verseText"$period $reference');
        // Always put double newline after reference to separate it from body text
        final replacement = '"$verseText$period"\n$reference\n\n';
        return replacement;
      },
    ...
    content = content.replaceAllMapped(
      ...
      (match) {
        ...
        if (kDebugMode)
          print(
              'DEBUG: Replacing single quote with space #$replacementCount: \'$verseText\'$period $reference');
        // Always put double newline after reference to separate it from body text
        final replacement = "'$verseText$period'\n$reference\n\n";
        return replacement;
      },
    ...
    if (kDebugMode)
      print(
          'DEBUG: Content after replacement (first $previewLength chars): ${content.substring(0, previewLength)}');

    // IMMEDIATELY identify and extract verse-reference pairs before splitting
    // This ensures they're never mixed with body text
    const verseRefPatternForExtract =
        r'((?:\d+\s+)?[A-Z][a-zA-Z]+(?:\s+of\s+[A-Z][a-zA-Z]+)?\s+(?:\d+:\d+(?:\s*[,\u2013\u2014-]\s*\d+)*|\d+))';
    ...
    if (firstVerseRefMatch != null) {
      ...
      if (kDebugMode)
        print(
            'DEBUG: [EXTRACT] Found verse-reference pair (no period): verse="$extractedVerse", ref="$extractedReference"');
    } else {
      ...
      if (firstVerseRefMatch != null) {
        ...
        if (kDebugMode)
          print(
              'DEBUG: [EXTRACT] Found verse-reference pair (with period): verse="$extractedVerse", ref="$extractedReference"');
      }
    }
    ...
    if (firstVerseRefMatch != null &&
      ...
      if (kDebugMode)
        print(
            'DEBUG: [EXTRACT] Removed verse-reference pair from content, remaining length: ${contentWithoutVerseRef.length}');
    }

    // Split by double newlines for paragraphs
    var paragraphs = contentWithoutVerseRef
        .split('\n\n')
        .map((p) => p.trim())
    ...
    const verseRefPatternForFilter =
        r'((?:\d+\s+)?[A-Z][a-zA-Z]+(?:\s+of\s+[A-Z][a-zA-Z]+)?\s+(?:\d+:\d+(?:\s*[,\u2013\u2014-]\s*\d+)*|\d+))';
    ...
    for (final para in paragraphs) {
      ...
      final sameLinePattern = RegExp(
          '^(["\'])([\\s\\S]+?)(["\'])(\\.?)[\\s\\-]+$verseRefPatternForFilter');
      ...
      final standaloneRefPattern =
          RegExp('^' + verseRefPatternForFilter + r'\$');
      if (standaloneRefPattern.hasMatch(para) &&
          initialRefForFiltering == null) {
    ...
    if (foundInitialRef && initialRefForFiltering != null) {
      final escapedRef = initialRefForFiltering.replaceAllMapped(
          RegExp(r'[.*+?^${}()|[\]\\]'), (m) => '\\${m[0]}');
      final refPattern =
          RegExp('^\\s*' + escapedRef + r'\\s*$', caseSensitive: false);
      ...
      paragraphs = paragraphs.where((para) {
        ...
        if (seenVerseRefPair) {
          ...
          if (trimmed == initialRefForFiltering ||
              refPattern.hasMatch(trimmed)) {
            removedCount++;
            if (kDebugMode)
              print(
                  'DEBUG: [PRE-FILTER] Removing duplicate reference paragraph #$removedCount after verse-ref pair: $trimmed');
            return false;
          }
          ...
          for (final line in lines) {
            ...
            if (lineTrimmed == initialRefForFiltering ||
                refPattern.hasMatch(lineTrimmed)) {
              // If it's a single-line paragraph that's just the reference, remove it
              if (lines.length == 1) {
                removedCount++;
                if (kDebugMode)
                  print(
                      'DEBUG: [PRE-FILTER] Removing single-line reference paragraph #$removedCount after verse-ref pair: $trimmed');
                return false;
              }
            }
          }
      ...
      if (kDebugMode)
        print(
            'DEBUG: [PRE-FILTER] Removed $removedCount duplicate reference paragraphs');
    }
    ...
    while (i < paragraphs.length) {
      ...
      if (isPotentialPoemLine && startsWithQuote) {
        ...
        while (j < paragraphs.length) {
          ...
          final isShortLine =
              currentPara.length < 100 && !currentPara.contains('\n');
          if (isShortLine) {
            poemLines.add(currentPara.trim());
            j++;
            // Stop if we hit a line that ends with a quote (end of poem)
            if (currentPara.trim().endsWith('"') ||
                currentPara.trim().endsWith("'") ||
                currentPara.trim().endsWith('".') ||
                currentPara.trim().endsWith("'.")) {
              break;
            }
      ...
      const verseRefPattern =
          r'((?:\d+\s+)?[A-Z][a-zA-Z]+(?:\s+of\s+[A-Z][a-zA-Z]+)?\s+\d+:\d+)';

      // Pattern 1: Check without period first (most common: "verse"\nreference)
      final verseRefMatchNoPeriod =
          RegExp('^(["\'])(.+?)(["\'])\\n' + verseRefPattern + r'\$')
              .firstMatch(para);
      // Pattern 2: Check with optional period ("verse".\nreference)
      final verseRefMatchWithPeriod =
          RegExp('^(["\'])(.+?)(["\']\\.?)\\n' + verseRefPattern + r'\$')
              .firstMatch(para);
      final hasVerseRef =
          verseRefMatchNoPeriod != null || verseRefMatchWithPeriod != null;
      ...
      final endsWithQuote = para.endsWith('"') ||
          para.endsWith("'") ||
          para.endsWith('".') ||
          para.endsWith("'.");
      ...
      if (!hasVerseRef && !isPoem && i + 1 < paragraphs.length) {
        ...
        if (!endsWithPunctuation) {
          ...
          if (!nextIsVerseRef && !nextStartsWithQuote) {
            ...
            final lastWords =
                trimmedPara.split(' ').where((w) => w.isNotEmpty).toList();
            final lastWord = lastWords.isNotEmpty
                ? lastWords.last.toLowerCase().replaceAll(RegExp(r'[^\w]'), '')
                : '';
            ...
            final nextFirstWords =
                nextPara.split(' ').where((w) => w.isNotEmpty).take(2).toList();
            final nextFirstWord = nextFirstWords.isNotEmpty
                ? nextFirstWords[0]
                    .toLowerCase()
    ...
    bool hasSeenInitialVerseRef =
        false; // Track if we've seen the initial verse-reference pair
    String?
        initialReference; // Store the initial reference to remove duplicates
    ...
    if (extractedVerse != null && extractedReference != null) {
      ...
      spans.add(TextSpan(
        ...
        semanticsLabel:
            bibleReferenceSemanticsLabel(displayReference) ?? displayReference,
        style: GoogleFonts.inter(
          fontStyle: FontStyle.italic,
          fontSize: 16,
          color: textColor,
      ...
      if (kDebugMode)
        print(
            'DEBUG: [OUTPUT] Output extracted verse-reference pair first: verse="$extractedVerse", ref="$extractedReference"');
    }
    ...
    for (int idx = 0; idx < combinedParagraphs.length; idx++) {
      ...
      if (!hasVerseRef) {
        final sameLinePattern = RegExp('^(["\'])(.+?)(["\'])(\\.?)[\\s\\-]+' +
            verseRefPattern2 +
            r'(.*)$');
        final sameLineMatch = sameLinePattern.firstMatch(para);
        if (sameLineMatch != null) {
      ...
      final simpleVerseRef = RegExp(
          r'^[A-Z][a-zA-Z]+\s+(?:\d+:\d+(?:\s*[,\u2013\u2014-]\s*\d+)*|\d+)$');
      final multiWordVerseRef = RegExp(
          r'^[A-Z][a-zA-Z\s]+(\s+of\s+[A-Z][a-zA-Z]+)?\s+(?:\d+:\d+(?:\s*[,\u2013\u2014-]\s*\d+)*|\d+)$');
      final numberedBookPattern = RegExp(
          r'^\d+\s+[A-Z][a-zA-Z\s]+\s+(?:\d+:\d+(?:\s*[,\u2013\u2014-]\s*\d+)*|\d+)$');
      final numberedWithOf = RegExp(
          r'^\d+\s+[A-Z][a-zA-Z\s]+(\s+of\s+[A-Z][a-zA-Z]+)?\s+(?:\d+:\d+(?:\s*[,\u2013\u2014-]\s*\d+)*|\d+)$');
    ...
    for (int index = 0; index < combinedParagraphs.length; index++) {
      ...
      if (hasSeenInitialVerseRef && initialReference != null) {
        final trimmedPara = para.trim();
        final escapedRef = initialReference.replaceAllMapped(
            RegExp(r'[.*+?^${}()|[\]\\]'), (m) => '\\${m[0]}');
        ...
        if (trimmedPara == initialReference) {
          if (kDebugMode)
            print(
                'DEBUG: [ULTRA-AGGRESSIVE] Skipping duplicate reference paragraph: $para');
          continue;
        }

        // Check if paragraph matches reference pattern (with optional whitespace)
        if (RegExp('^\\s*' + escapedRef + r'\\s*$', caseSensitive: false)
            .hasMatch(trimmedPara)) {
          if (kDebugMode)
            print(
                'DEBUG: [ULTRA-AGGRESSIVE] Skipping paragraph matching reference pattern: $para');
          continue;
        }
        ...
        if (RegExp('^\\s*' + escapedRef + r'\\s+', caseSensitive: false)
          ...
          if (para.isEmpty) {
            if (kDebugMode)
              print(
                  'DEBUG: [ULTRA-AGGRESSIVE] Paragraph became empty after removing reference, skipping: $trimmedPara');
            continue;
          }
        ...
        for (final line in lines) {
          ...
          if (trimmedLine == initialReference) {
            ...
            if (kDebugMode)
              print(
                  'DEBUG: [ULTRA-AGGRESSIVE] Removing reference line from paragraph: $trimmedLine');
          ...
          if (RegExp('^\\s*' + escapedRef + r'\\s*$', caseSensitive: false)
            ...
            if (kDebugMode)
              print(
                  'DEBUG: [ULTRA-AGGRESSIVE] Removing reference line (pattern match): $trimmedLine');
            continue; // Skip this line
          }
        ...
        if (foundRefLine) {
          ...
          if (para.isEmpty) {
            if (kDebugMode)
              print(
                  'DEBUG: [ULTRA-AGGRESSIVE] Paragraph became empty after removing reference lines, skipping');
            continue;
          }
        }
      }
      ...
      if (!hasVerseRef) {
        ...
        if (noPeriodMatch != null) {
          ...
          if (kDebugMode)
            print(
                'DEBUG: Found verse-reference pair (no period pattern): $para');
        }
      }
      ...
      if (!hasVerseRef) {
        ...
        if (sameLineMatch != null) {
          ...
          if (quoteChar != null && verseText != null && reference != null) {
            para =
                '$quoteChar$verseText$period$quoteChar\n$reference$restOfPara';
            hasVerseRef = true;
            verseRefMatch = verseRefPatternRegex.firstMatch(para);
            if (kDebugMode)
              print(
                  'DEBUG: Found verse-reference on same line, split it: $para');
          }
        }
      }
      ...
      if (hasVerseRef && verseRefMatch != null) {
        if (kDebugMode)
          print(
              'DEBUG: Detected verse-reference pair: verse="${verseRefMatch.group(2)}", ref="${verseRefMatch.group(4)}"');
      }

      // Check if it's a quoted scripture verse
      final startsWithQuote2 = para.startsWith('"') || para.startsWith("'");
      final endsWithQuote2 = para.endsWith('"') ||
          para.endsWith("'") ||
          para.endsWith('".') ||
          para.endsWith("'.");
      final isQuotedVerse =
          startsWithQuote2 && endsWithQuote2 && para.length < 600 && index < 8;

      // Check if it's a scripture verse reference
      final simpleVerseRef = RegExp(r'^[A-Z][a-zA-Z]+\s+\d+:\d+$');
      final multiWordVerseRef =
          RegExp(r'^[A-Z][a-zA-Z\s]+(\s+of\s+[A-Z][a-zA-Z]+)?\s+\d+:\d+$');
      final numberedBookPattern = RegExp(r'^\d+\s+[A-Z][a-zA-Z\s]+\s+\d+:\d+$');
      final numberedWithOf = RegExp(
          r'^\d+\s+[A-Z][a-zA-Z\s]+(\s+of\s+[A-Z][a-zA-Z]+)?\s+\d+:\d+$');
      ...
      final isEarlyQuoted = startsWithQuote3 &&
          index < 2 &&
          para.length < 400 &&
          !para.contains('\n');
      ...
      final isScripture = !hasSeenInitialVerseRef &&
          (isVerseRef || isQuotedVerse || isEarlyQuoted);
      ...
      if (hasSeenInitialVerseRef && initialReference != null) {
        ...
        final lines = para.split('\n');
        final isJustReference =
            lines.length == 1 && lines[0].trim() == initialReference;
        if (isJustReference) {
          if (kDebugMode)
            print(
                'DEBUG: Skipping paragraph that is just the reference: $para');
          continue;
        }
        ...
        if (startsWithRef) {
          if (kDebugMode)
            print(
                'DEBUG: Skipping paragraph that starts with reference: $para');
          continue;
        }
      }
      ...
      final nonEmptyParaLines =
          paraLines.where((line) => line.trim().isNotEmpty).toList();
      ...
      final lastLineEndsQuote = nonEmptyParaLines.isNotEmpty &&
          (nonEmptyParaLines.last.trim().endsWith('"') ||
              ...
              nonEmptyParaLines.last.trim().endsWith("'."));
      final allLinesShort =
          nonEmptyParaLines.every((line) => line.trim().length < 100);
      ...
      if (hasVerseRef) {
        ...
        if (verseRefParts.length >= 2) {
          ...
          spans.add(TextSpan(
            text: '$displayReference\n\n',
            semanticsLabel: bibleReferenceSemanticsLabel(displayReference) ??
                displayReference,
            style: GoogleFonts.inter(
              fontStyle: FontStyle.italic,
              fontSize: 16,
        ...
        if (!hasSeenInitialVerseRef) {
          ...
          final displayPara =
              isVerseRef ? _normalizeReferenceDisplay(para) : para;
          spans.add(TextSpan(
            text: '$displayPara\n\n',
            semanticsLabel: isVerseRef
                ? (bibleReferenceSemanticsLabel(displayPara) ?? displayPara)
                : null,
            style: GoogleFonts.inter(
              fontStyle: FontStyle.italic,
              fontSize: 16,
      ...
      } else if (hasSeenInitialVerseRef &&
        ...
        if (kDebugMode)
          print(
              'DEBUG: Skipping duplicate reference that appears as separate paragraph: $para');
        continue;
      } else if (isPoem) {
        ...
        if (hasSeenInitialVerseRef && initialReference != null) {
          ...
          final startsWithRefPattern = RegExp(
              '^\\s*' + escapedRef + r'(?:\s+' + escapedRef + r')*\s*',
              caseSensitive: false);
          ...
          for (final line in lines) {
            ...
            if (processedLine == initialReference) {
              if (kDebugMode)
                print(
                    'DEBUG: Removing duplicate reference line from body: $processedLine');
              continue;
            }
            ...
            if (RegExp(
              ...
              if (kDebugMode)
                print(
                    'DEBUG: Removing duplicate reference line (repeated) from body: $processedLine');
              continue;
            }
            // Also remove the reference if it appears at the start of a line (even if there's more text)
            final lineStartsWithRef =
                RegExp('^\\s*' + escapedRef + r'\\s+', caseSensitive: false);
            if (lineStartsWithRef.hasMatch(processedLine)) {
              processedLine =
                  processedLine.replaceFirst(lineStartsWithRef, '').trim();
              if (processedLine.isEmpty) {
                if (kDebugMode)
                  print(
                      'DEBUG: Removing line that starts with reference from body: $line');
                continue;
              }
            }
          ...
          final standaloneRefPattern = RegExp(
              r'(?:^|\s+)' + escapedRef + r'(?:\s+|$)',
              caseSensitive: false);
          para = para
              .replaceAll(standaloneRefPattern, ' ')
              .replaceAll(RegExp(r'\s+'), ' ')
              .trim();
          if (para.isEmpty) continue;
        }
